    ...
    for sₜ ∈ [0,1] #retired or not
        if sₜ==0  #retired  
            if t==T 
                ...
                global M̄_r
                global v_r
                (c_fct_r[t],v_fct_r[t],M̄_r,v_r)=EGM_retired(c_fct_r[t+1],v_fct_r[t+1])
            end
        else #not retired
            for dₜ ∈ [0,1] #can decide to work or not
                if t==T
                    ...
                    if dₜ==0
                        ...
                        (cʷ[t,:,dₜ+1],Vʷ[t,:,dₜ+1],M̄_d[t,:,dₜ+1])=EGM_worker(c_fct_d[t+1,:],v_fct_d[t+1,:])
...
t=T-2
dₜ=1
scatter(A_grid,c_fct_d[1,dₜ+1].(A_grid),label="v_d",color=:black,markersize=2)
scatter!(A_grid,c_fct_d[10,dₜ+1].(A_grid),label="v_d",color=:red,markersize=2)
scatter!(A_grid,c_fct_d[18,dₜ+1].(A_grid),label="v_d",color=:blue,markersize=2)
xlims!(120,400)
ylims!(20,30)
